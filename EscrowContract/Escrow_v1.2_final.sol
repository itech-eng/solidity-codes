...
contract Escrow {
    ...
    mapping (string => bytes32) public pendingTrades;
    ...
    uint256 _totalFee, string memory _note) external onlyAdmin returns(bool) {
        ...
        require(_receiver != msg.sender, "Admin can't be a receiver.");
    ...
    function addEscrowRecord(address _tokenContract, string memory _tradeId, address _seller, address _buyer, 
    uint256 _amountWithFee, uint256 _totalFee) external onlyAdmin returns(bool) {
        ...
        require(pendingTrades[_tradeId] == bytes32(0), "Trade id already exists.");

        pendingTrades[_tradeId] = keccak256(abi.encode(_tokenContract, _seller, _buyer, _amountWithFee, _totalFee));
    ...
    function releaseAmount(address _tokenContract, string memory _tradeId, address _seller, address _buyer, 
    uint256 _amountWithFee, uint256 _totalFee, bool _success) external adminOrOwner returns(bool) {
        
        require(pendingTrades[_tradeId] == keccak256(abi.encode(_tokenContract, _seller, _buyer, _amountWithFee, _totalFee))
        , "Trade Id and other param data are not valid.");
        
        address _payee = _seller;

        require(uint256(_amountWithFee) <= escrowFund[_tokenContract][_seller], 
        "Invalid _amountWithFee, Not enough balance in Escrow fund of this seller.");
        
        uint256 amountToSend = uint256(_amountWithFee);
        if(_success) {
            _payee = _buyer;
